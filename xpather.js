...
function createSidebarEntry(index, node, type) {
	...
	var entryContent = '<div class="{className}" value="{text0}">{text}{fade}<div class="xpather-sidebar-entry-count">{i}</div></div>'.supplant({
...
function copyResultToClipboard(entries) {
	
	var nodelist = entries.children('.xpather-sidebar-entry');
	var elements = Array.from(nodelist);
	var result = elements.map(item => item.getAttribute("value")).join("\n\n");	
	var $temp = $("<textarea>");
    $("body").append($temp);
...
if (isDocumentValid) {
	...
	var $sidebarCopy = $('#xpather-sidebar-copy');
	...
	$sidebarCopy.click(function () {
		'use strict';
		copyResultToClipboard($sidebarEntries);
	});
